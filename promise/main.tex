...
Last, we present an implementation of the protocol on Ethereum showing that all functions of the protocol can be implemented in constant time complexity and \sys only adds USD 0.01 per service delivery during the subscription period.
...
Further, \sys is essentially a subscription mechanism:
...
Moreover, we show that the implementation of \sys in an Ethereum Solidity smart contract only adds USD 0.03 to setup Promise between a user and a service provider, USD 0.01 to proide the deposit for the service provider and USD 0.01 to provide the pre-payment.
...
\begin{itemize}
    \item \textbf{Alice}, the Intermediary: Alice is economically rational and entrusted with executing a task. She provides a deposit $D$ into the escrow before executing the task and receives $m$ payments $p$ upon successful completion.
    \item \textbf{Bob}, the User: Bob represents the user requesting execution of a task by Alice. A user provides payments $\{p_1, ..., p_m\}$ into the escrow. % Bob can provide multiple future payments into the escrow.
...
Therefore, Alice fulfills the specification iff $p > V_A -D$. %$u_A(A=H)>u_A(A=C)$, or equivalently 
Assuming that $V_B-p-c>0$, otherwise Bob would not seek the service from Alice in the first place, he stands to gain utility if Alice provides the service. %.; if Alice is malicious he will receive a payoff of 0.
...
\begin{figure}[t!]
    ...
    \caption{\sys allows intermediaries (Alice) to lock less initial deposit $D_I$ and use payments $p_i$ provided by users (Bob) as additional deposit. The initial deposit and payments are locked until time $m$ determined by Bob. Only when Alice behaves honestly until $m$ can Alice withdraw her initial deposit $D_I$ and the total payments $\sum_{t=0}^{m}p_t$.}
    \label{fig:promise}
\end{figure}
...
\begin{enumerate}
    \item At $t=0$: $B$ locks $m+1$ payments in $\mathrm{P}$. $A$ locks the initial deposit $D_I$ in $\mathrm{P}$.
...
To argue about the security of \sys, we introduce two concepts: (i) discounting and (ii) a likelihood of users exiting the system upon the counterparty cheating.

\subsection{Introducing Discounting}
Discounting is also introduced, where $0<\delta<1$ denotes the discount factor of an agent's valuation of future utility. 
This captures the notion that the future is worth less than the present.
...
By setting large $m$ and being able to quit the protocol upon Alice's misbehavior, he can motivate rational Alice to behave honestly.
...
\begin{enumerate}
    ...
    \item \textbf{Sybil resistance}: Alice cannot increase her pay-off $u_A$ by creating multiple Sybil identities $A'$ to increase.
\end{enumerate}
